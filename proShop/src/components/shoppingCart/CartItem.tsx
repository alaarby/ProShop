...
const CartItem = ({ product }: CartItemProp) => {
  ...
  return (
    <div className="flex bg-[#F2F2F2] xl:min-w-[800px] rounded-[16px] relative pr-[35px]">
      <button 
      ...
      <img 
        src={product.image} 
        alt={product.name}
        className="w-[200px] lg:w-[304px] h-[100px] lg:h-[203px] mt-[21px] ml-[13px]"
      />
      <div className="grid grid-cols-1 md:grid-cols-3 flex-1">
        <h3 className="text-[24px] text-[#242424] font-semibold self-start mt-[51px]">
          {product.name}
        </h3>
        <div className="flex items-center justify-between xl:max-w-[204px] h-[40px] bg-white self-center">
          <button 
        ...
        </div>
        {
          discount > 0 ? (
            <div className="flex flex-col md:items-end self-center">
              <span className="text-neutral-500 test-[20px] line-through md:text-right">
                ${(product.price).toFixed(2)}
              </span>
              <span className="text-[24px] md:text-[30px] text-neutral-800 font-bold md:text-right">
                ${(product.price * (1 - discount / 100)).toFixed(2)}
              </span>
            </div>
          ): (
            <span className="text-[24px] md:text-[30px] text-neutral-800 font-bold md:text-right self-center">
              ${(product.price).toFixed(2)}
            </span>
          )
        }
